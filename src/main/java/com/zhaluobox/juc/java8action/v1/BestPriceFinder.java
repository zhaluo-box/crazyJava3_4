package com.zhaluobox.juc.java8action.v1;
...
import static com.zhaluobox.juc.java8action.ExchangeService.Money;
import static com.zhaluobox.juc.java8action.ExchangeService.getRate;
        ...
        List<CompletableFuture<String>> priceFutures =
                shops.stream()
                        .map( shop -> CompletableFuture.supplyAsync( () -> shop.getName() + " price is "
                                + shop.getPrice( product ), executor ) ) // 指定了自定义的线程池 , 采用fixed 固定线程的线程池.
                        .collect( Collectors.toList() );

        List<String> prices = priceFutures.stream()
                .map( CompletableFuture::join )  // 等待list<CompletableFuture> 全部执行结束.
                .collect( Collectors.toList() );
        return prices;
    }
