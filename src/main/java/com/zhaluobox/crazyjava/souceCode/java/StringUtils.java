 ...
 */
package com.zhaluobox.crazyjava.souceCode.java;
...
public class StringUtils {
     ...
     */
	 ...
	 /*----字符串为null 或 长度为 0 或者全是空格 都返回true 反之-----*/
    public static boolean isBlank(String str) {
        int strLen;
		/*--如果str 为null 或者 str长度为0 则返回true 反之..--*/
    ...
    public static String stripStart(String str, String stripChars) {
        int strLen;
		/*--如果字符串为null 或者 字符串长度为0 返回字符串本身*/
        if (str == null || (strLen = str.length()) == 0) {
            return str;
        }
        int start = 0;
		/*--假如 stripChars 为null 则 判断字符串开始有几个空格 start=空格数 -- */
        if (stripChars == null) {
            while ((start != strLen) && Character.isWhitespace(str.charAt(start))) {
                start++;
            }
        } else if (stripChars.length() == 0) { /*--假如stripChars长度为0,则返回字符串本身--*/
            return str; 
        } else {
			/*--当 start 不等于 字符串长度, 且 stripCHars 按序对比源字符串的字符是否存在于 strip中.存在++ 不存在就直接截取源字符串 */
     ...
     * </pre>
     *
     * @param str  the String to remove characters from, may be null 被移除的字符串可以为null
     * @param stripChars  the set of characters to remove, null treated as whitespace null 按照空格处理
     * @return the stripped String, <code>null</code> if null String input
     */
    ...
    public static boolean equals(String str1, String str2) {
		/*
			...
			这个写法相当的巧妙的.
			我猛地一看.当时就感觉写的怪怪的.但是细的一看.感觉很精妙啊.思路非常的牛逼
			首先判断 : str1 == null;
				true则判断 str2 == null ,为null 则返回true,不为null,则直接返回false布尔值,都不需要再执行String的equals方法
				false则,走str1.equals(str2);
			但是看到[str1.equals(str2);],我又在想.这不是脱裤子放屁.多此一举么.
			OK,我去看一眼StringEquals的方法
				然后发现String重写的equals方法
				String的equals可以比较任何类型.
				而且没有判断null
			StringUtils 只能比较两个String类型
		*/
     ...
     * </pre>
     *
	 * @param str  the String to check, may be null 源字符串
     * @param searchChar  the character to find 要查找的字符
     * @param startPos  the start position 开始查找的索引
     * @return the last index of the search character,
     *  -1 if no match or <code>null</code> string input
     * 返回要查询的字符最后出现的索引
     * 如果源字符串为null 直接返回-1
     ...
     * @param startPos  the start position, negative treated as zero
     * @return the first index of the search String,
     *  -1 if no match or <code>null</code> string input 要查询字符最后出现索引位置.从startpos索引开始查询
     * @since 2.0
     */
...
//                    }
     ...
     * @since 2.0
     */
	 /*.从字符串中找出不属于字符数组的第一个字符出现的索引位置 (从0位开始)如果都在，返回-1*/
...
//            }
    ...
    public static boolean containsOnly(String str, char[] valid) {
		...
		/*-如果字符数组长度 为 0 返回false-*/
        if (valid.length == 0) {
            return false;
        }
		/*从字符串中找出不属于字符数组的第一个字符出现的索引位置 (从0位开始)如果都在，返回-1为false*/
		/*--总的来讲就是 如果字符串全部是由字符数组的字符组成的就返回true ,反之 false--*/
...
//                    }
     ...
     * </pre>
     *
     * @param str  the String to abbreviate, may be null 字符串可以为null
     * @param middle the String to replace the middle characters with, may be null 缩略的字符串也可以为null
     * @param length the length to abbreviate <code>str</code> to. 缩略的长度
     * @return the abbreviated String if the above criteria is met, or the original String supplied for abbreviation.
     * @since 2.5
     */
    public static String abbreviateMiddle(String str, String middle, int length) {
		/*如果Str为空 或者 middle 为空 则返回 字符串本身*/
        if (isEmpty(str) || isEmpty(middle)) {
            return str;
        }
      
	  	/*---如果缩略长度大于等于字符串长度 或 缩略长度 小于 缩略字符串长度+2 返回字符串本身 ----*/
